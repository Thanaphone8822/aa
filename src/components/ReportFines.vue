...
<template>
  <div class="bg-gray-300 text-black p-6">
    ...
    <ul v-if="!loading && !error" class="space-y-3">
      <li v-for="fine in filteredFineReports" :key="fine.id" class="bg-white p-4 rounded-lg shadow flex justify-between items-center">
        <span>Fine issued at: {{ fine.timestamp }} (ID: {{ fine.id }})</span>
        <button @click="toggleSummary(fine)" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition-colors">
          {{ selectedFine && selectedFine.id === fine.id ? 'ປິດ' : 'ເບິ່ງລາຍລະອຽດ' }}
        </button>
      </li>
    </ul>
    <div v-if="!loading && filteredFineReports.length === 0 && fineReports.length > 0" class="text-center text-gray-500 py-4">
  ...
  <!-- Modal Overlay -->
  <div v-if="selectedFine" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" @click="closeSummaryModal">
      ...
      <div class="p-6">
          ...
          <div class="overflow-x-auto">
            <table class="w-full">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr v-else v-for="(article, index) in selectedFine.articles" :key="article.id" class="hover:bg-gray-50">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-gray-50 p-6 rounded-lg shadow">
            ...
            <textarea 
              v-model="selectedFine.complaintDetails"
        ...
        <div class="mt-6 flex justify-end space-x-3">
          <button @click="closeSummaryModal" class="px-6 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors">
            ປິດ
          </button>
        </div>
      </div>
